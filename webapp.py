...
async def yookassa_webhook(request: web.Request) -> web.Response:
    """Уведомление об оплате рублями. НЕ доверяем телу вебхука напрямую (его
    легко подделать POST-запросом на публичный урл) — переспрашиваем статус
    и сумму у самой ЮKassa по payment_id (fetch_payment, сервер-сервер) и
    только тогда выдаём премиум. mark_yookassa_payment — идемпотентность:
    ЮKassa повторяет вебхук, пока не получит 200, повторная обработка того
    же payment_id не должна выдать премиум дважды."""
    ...
    if payload == PREMIUM_PAYLOAD:
        ...
        try:
            await _bot.send_message(
                ...
                "Через месяц пришлю напоминание продлить (это разовый платёж, "
                "не автоподписка, как со звёздами)."
            )
        except Exception as e:
