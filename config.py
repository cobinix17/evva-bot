...
# Повторная генерация КУПЛЕННОГО разбора на ДРУГУЮ дату (посмотреть подругу,
# партнёра и т.д.). Первая генерация после покупки лимит не тратит. Ограничение
# чисто анти-абузное: каждая генерация — полный запрос к ИИ, а платят один раз.
# Потолок стоит выше самого частого сценария (человек смотрит себя + пару
# близких), потому что новые даты и так упираются в платные слоты ниже —
# этот лимит остаётся лишь страховкой от абьюза, а не основным ограничителем.
REGEN_DAILY_LIMIT         = 10
REGEN_DAILY_LIMIT_PREMIUM = 25     # у подписчиков потолок выше — они платят ежемесячно

# ─── РАЗБОР НА ДРУГУЮ ДАТУ ───────────────────────────────────────────────────
# Один купленный разбор считается по ОДНОЙ дате рождения. Посмотреть по этому
# же разбору другого человека (партнёра, ребёнка, подругу) — отдельная покупка
# со скидкой: человек уже внутри и уже доверяет, порог второй покупки должен
# быть ниже первой.
# Приветственные звёзды тому, КОГО пригласили. Приглашающий получает процент с
# покупок (REF_BONUS_PERCENT), но без выгоды для второй стороны приглашение
# выглядит так, будто человека используют. Сумма намеренно меньше самого
# дешёвого разбора: это скидка на первую покупку, а не бесплатный разбор,
# поэтому фермить фейковые аккаунты ради неё бессмысленно.
REF_WELCOME_BONUS = 20             # ⭐ на баланс новому пользователю по ссылке

REDATE_DISCOUNT = 30               # % скидки на разбор для другого человека
REDATE_PREFIX   = "redate_"        # префикс payload оплаты и callback_data

def redate_price(key: str, default: int = 49) -> int:
    """Цена разбора на ДРУГУЮ дату — со скидкой от текущей цены (то есть
    складывается с общей акцией, если она включена)."""
    return max(1, round(price_of(key, default) * (100 - REDATE_DISCOUNT) / 100))
