...
# Сколько последних пар «вопрос-ответ» уходит в промпт уточнения. Больше не
# нужно: сам текст разбора там же, и длинная история вытесняла бы его.
FOLLOWUP_HISTORY      = 3
...
# Награда за отзыв. Начисляется ТОЛЬКО после публикации (модерации), а не за
# отправку: иначе выгодно прислать «спасибо» ради звёзд, и модерация тонет в
# мусоре. Отзыв можно оставить один раз на разбор, а разборы надо сначала
# получить — поэтому накрутить награду, ничего не купив, нельзя.
REVIEW_BONUS = 10                  # ⭐ за опубликованный отзыв
...
RAZBOR_DESCRIPTIONS = {
    ...
    "mission":       "Твоя жизненная миссия — зачем ты здесь.",
