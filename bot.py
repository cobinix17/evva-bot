...
def get_user(user_id):
    ...
    if user_id == ADMIN_ID:
        users[user_id]["free_used"] = True
        users[user_id]["subscribed_channel"] = True
        for r in ["compat", "when", "portrait", "unlucky", "matrix", "mission", "karma", "career", "money", "days"]:
            if r not in users[user_id]["purchased"]:
                users[user_id]["purchased"].append(r)
...
async def generate_pdf(text: str, title: str) -> bytes:
    try:
        ...
        # Попытка загрузить шрифт с кириллицей
        ...
        if os.path.exists(font_path):
            ...
            logging.warning("Unicode font not found, falling back to ASCII (Cyrillic will be lost)")
        ...
        return pdf.output(dest='S').encode('latin-1') if not os.path.exists(font_path) else pdf.output()
...
async def handle_two_dates(message: Message, state: FSMContext):
    ...
    try:
        ...
        pdf_bytes = await generate_pdf(answer, "Совместимость двух людей — Ева Нумеролог")
        if pdf_bytes:
        ...
        logging.error(f"Error in compat handler: {e}")
...
async def handle_date(message: Message, state: FSMContext):
    user = get_user(message.from_user.id)
    text = message.text.strip()
    if not all(c.isdigit() or c == "." for c in text) or len(text) < 8:
    ...
    try:
        if waiting == "free":
            ...
            filename = "karmicheskiy_dolg.pdf"
        elif waiting == "career":
            ...
            filename = "silnye_dni.pdf"
        else:
            await state.clear()
            return
        answer = await ask_groq(prompt)
        await message.answer(f"{title}\n\n{answer}")
        pdf_bytes = await generate_pdf(answer, f"{title} — Ева Нумеролог")
        if pdf_bytes:
            await message.answer_document(
        ...
        logging.error(f"Error in date handler ({waiting}): {e}")
        await message.answer("Произошла ошибка, попробуй ещё раз.")
    await state.clear()

@dp.callback_query(F.data == "leave_review")
async def leave_review(callback: CallbackQuery, state: FSMContext):
    await callback.message.answer("💬 Напиши свой отзыв — я передам его в наш канал!")
        ...
        for user_id in user_ids:
            ...
            if user.get("birth_date") and user.get("destiny_number"):
                try:
                    ...
                    logging.error(f"Daily horoscope failed for user {user_id}: {e}")
