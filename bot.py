...
SYSTEM_PROMPT = (
    ...
    "ОБЪЁМ И ПОЛНОТА — важно соблюсти ВСЮ структуру, которую просит запрос:\n"
    "Если в запросе дан список emoji-заголовков — ответь по КАЖДОМУ из них, "
    "не пропускай ни один и не объединяй несколько в один. Лучше короче по "
    "каждому пункту, но пройти всю структуру целиком, чем подробно ответить "
    "только на последний пункт. Каждый блок — несколько предложений, по "
    "существу. Не растягивай искусственно и не повторяй один и тот же "
    "блок дальше в тексте — каждый emoji-заголовок встречается в ответе "
    "ровно один раз.\n\n"
