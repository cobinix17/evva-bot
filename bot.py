...
if not any([os.getenv("CEREBRAS_API_KEY"), os.getenv("GROQ_API_KEY"),
    ...
    raise EnvironmentError(
        "Нужен хотя бы один ИИ-провайдер: CUSTOM_API_KEY + CUSTOM_BASE_URL, "
        "EXPERIENTIAL_API_KEY, CEREBRAS_API_KEY, GROQ_API_KEY или OPENROUTER_API_KEY."
    )
...
# Регистрируется САМЫМ ПОСЛЕДНИМ: aiogram перебирает обработчики в порядке
# объявления, поэтому все команды и состояния разбирают свои сообщения раньше,
# а сюда падает только то, что не подошло никому.
#
# Без него любое неожиданное сообщение уходило в пустоту: в логах остаётся
# «Update ... is not handled», а человек видит молчание и решает, что бот
# сломался. Особенно легко попасть после перезапуска бота — состояние диалога
# живёт в памяти процесса и после рестарта теряется, так что дата рождения,
# отправленная в этот момент, просто исчезала.
@dp.message()
async def fallback_handler(message: Message, state: FSMContext):
    user = await db.get_user(message.from_user.id)
    text = (message.text or "").strip()

    # Похоже на дату — значит человек отвечал на вопрос бота, а состояние
    # потерялось. Не отправляем его в меню молча, а объясняем, что делать.
    if text and is_valid_date(normalize_date(text)):
        await message.answer(
            "Вижу дату, но не помню, к какому разбору она была 🙈\n\n"
            "Так бывает, если я перезапускалась. Выбери разбор в меню и "
            "пришли дату ещё раз — она не потеряется.",
            reply_markup=main_menu_for(message.from_user.id, user)
        )
        return

    await message.answer(
        "Не поняла эту команду 🙂 Вот что я умею — выбирай в меню:",
        reply_markup=main_menu_for(message.from_user.id, user)
    )
