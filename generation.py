...
async def answer_ask(user_id: int, user: dict, question: str) -> str:
    ...
    prompt = (
        ...
        "Тон тёплый, живой, как у близкого человека, который давно тебя знает. Пиши простыми точными словами; перед "
...
def _gender_note(user: dict, subject_male: bool | None = "owner") -> str:
    """Префикс к промпту, задающий род. SYSTEM_PROMPT по умолчанию требует
    женский, но содержит явное исключение по этой фразе — шаблоны разборов
    остаются без изменений, род переключается одной строкой.

    subject_male: True/False — пол ТОГО, О КОМ разбор; None — пол неизвестен;
    "owner" (по умолчанию) — разбор о самом владельце аккаунта.

    Раньше род всегда брался у владельца. Разбор на дочь, заказанный отцом,
    приходил в мужском роде: «ты родился», «привлекаешь партнёршу». Пол
    субъекта и пол заказчика — разные вещи, и путать их нельзя.
    """
    if subject_male == "owner":
        subject_male = db.is_male(user)
    if subject_male is None:
        # Честнее ничего не утверждать, чем угадать неверно: в русском почти
        # любое прошедшее время и прилагательное несут род, поэтому просим
        # обходить их, а не «писать нейтрально» в общем виде.
        return ("ВАЖНО: пол человека, о котором разбор, НЕИЗВЕСТЕН. Не пиши о нём "
                "ни в мужском, ни в женском роде: избегай прошедшего времени "
                "('ты родился/родилась') и прилагательных о нём "
                "('сильный/сильная'). Пиши настоящим временем и "
                "существительными: 'в тебе есть сила', 'твоя опора — ...'. "
                "Про партнёра говори нейтрально: 'твой человек', 'партнёр'. "
                "Ниже в задании клиент может быть описан словами «она», «её» — "
                "это шаблон, не переноси его род в ответ.\n\n")
    if subject_male:
        return ("ВАЖНО: разбор о МУЖЧИНЕ. Обращайся к нему только в мужском роде "
                "('ты родился', 'ты пришёл', 'ты сильный'), о партнёре пиши в "
                "женском роде ('твоя партнёрша', 'твоя женщина'). "
                # Сами тексты разборов написаны про женщину: в описаниях блоков
                # сплошь «её числа», «что она делает», «передать именно ей».
                # Без этой строки модель послушно повторяла женский род за
                # инструкцией, даже зная, что клиент мужчина.
                "И ещё: ниже в задании клиент описан словами «она», «её», «ей», "
                "«женщина» — это шаблон, читай их как «он», «его», «ему», "
                "«мужчина». Ни одного женского слова о самом клиенте в ответе "
                "быть не должно.\n\n")
    return ("ВАЖНО: разбор о ЖЕНЩИНЕ — обращайся только в женском роде "
            "('ты родилась', 'ты пришла'), о партнёре пиши в мужском "
            "('твой партнёр').\n\n")
...
async def generate_single(user_id: int, user: dict, key: str, date_str: str,
    ...
    try:
        ...
        prompt  = _gender_note(user, subject_male) + build_prompt(key, name=name, context=context, date=date_str)
        try:
            async with premium_gen_semaphore(user):
                answer = await ask_ai(prompt)
        except Exception:
