...
async def answer_ask(user_id: int, user: dict, question: str) -> str:
    ...
    prompt = (
        f"Ты — Ева, тёплый личный нумеролог-помощник {name}. Ты знаешь её числа, "
        f"текущий период и разборы, которые уже для неё делала.\n\n"
        ...
        "постороннюю тему) — по-доброму скажи, что это не по твоей части. Тон тёплый, "
        "живой, как близкая подруга. Без emoji-заголовков, без блоков — связный текст, "
        "обычно 2-6 предложений."
    )
