...
from header_emoji import HEADER_EMOJI as _HEADER_EMOJI
...
# Именительный падеж — для перечня разрешённых месяцев в _today_note.
_RU_MONTHS_NOM = (
...
def _today_note() -> str:
    ...
    Правило «не называй прошедшие месяцы» модель нарушала: в сентябре 2026
    она советовала «март-апрель 2026». Поэтому вместо правила даём готовый
    перечень — выбрать из списка ей проще, чем сравнивать даты."""
    now = datetime.now()
    allowed = [f"{_RU_MONTHS_NOM[m - 1]} {now.year}" for m in range(now.month, 13)]
    allowed += [f"{_RU_MONTHS_NOM[m - 1]} {now.year + 1}" for m in range(1, 13)]
    return (
        f"СЕГОДНЯШНЯЯ ДАТА: {now.day} {_RU_MONTHS[now.month - 1]} {now.year} года. "
        f"Когда советуешь месяцы для действий, прогнозируешь события или "
        f"называешь лучшее время — разрешено называть ТОЛЬКО месяцы из этого "
        f"списка: {', '.join(allowed)}. Любой другой месяц уже прошёл — "
        f"называть его нельзя.\n\n"
    )
...
SYSTEM_PROMPT = (
    ...
    "Ты делаешь платные разборы — каждый разбор должен быть "
    "конкретным и ценным, без лишней воды.\n\n"
    "АБСОЛЮТНОЕ ПРАВИЛО — РОД ОБРАЩЕНИЯ:\n"
    "По умолчанию клиент — женщина. Обращайся в женском роде: "
    "'ты родилась', 'ты пришла', 'ты решила', 'ты замужем', 'твой партнёр'. "
    "ЗАПРЕЩЕНО писать женщине: 'ты родился', 'ты пришёл', 'ты замкнут', 'ты верный'. "
    "ИСКЛЮЧЕНИЕ: если в задании явно сказано «клиент — мужчина», то ВСЁ наоборот — "
    "обращайся только в мужском роде ('ты родился', 'ты пришёл', 'ты сильный', "
    "'ты женат', 'твоя партнёрша', 'твоя женщина') и не используй женские формы о нём. "
    "Все упоминания женского рода о клиенте в остальных правилах в этом случае "
    "читай как мужской род. Это правило важнее всего остального — нарушение недопустимо.\n\n"
    "ГОЛОС И СТИЛЬ:\n"
    ...
    "Хорошо: 'следующие два месяца — пиковый период для карьерных решений' "
    "(с конкретными названиями месяцев из разрешённого списка выше). "
    "Называй конкретные числа, месяцы, паттерны, ситуации.\n\n"
    "ОБЪЁМ И ПОЛНОТА:\n"
    "Если в запросе дан список emoji-заголовков — ответь по КАЖДОМУ из них, "
    "не пропускай ни один и не объединяй несколько в один. Каждый emoji-заголовок "
    "встречается в ответе ровно один раз.\n"
    "Каждый блок раскрывай содержательно — несколько развёрнутых предложений с "
    "конкретными числами, месяцами и примерами, а не одной короткой фразой. "
    "Целевой объём в задании — ориентир, а не норма выработки: полнота структуры "
    "важнее количества слов. Не растягивай повторами и не обрывай последний блок "
    "ради того, чтобы уложиться.\n\n"
    ...
    "Обращайся только на ТЫ — никогда не пиши 'вы', 'ваш'. "
    "Имя пользователя — используй только то что указано в данных, не придумывай другое. "
    "Упоминай имя не чаще 3 раз за весь текст. "
    "ВАЖНО — пол пользователя: по умолчанию женский род — 'ты пришла', "
    "'ты решила', 'твоя миссия', 'партнёр' (о мужчине рядом с ней). "
    "Но если в задании сказано «клиент — мужчина», действует ИСКЛЮЧЕНИЕ "
    "из правила о роде выше: тогда весь текст в мужском роде, и это правило "
    "тоже читается наоборот.\n\n"
    "ЗАГОЛОВКИ БЛОКОВ:\n"
...
def _is_emoji(ch: str) -> bool:
    """Эмодзи или его модификатор. Обрыв генерации всегда приходится на букву
    посреди слова, а не на эмодзи, поэтому эмодзи в конце — признак того, что
    модель закончила мысль сама."""
    o = ord(ch)
    return (
        0x1F000 <= o <= 0x1FAFF      # основные блоки эмодзи
        or 0x2600 <= o <= 0x27BF     # symbols & dingbats: ✨ ☀ ✅
        or o in (0xFE0F, 0x2B50, 0x2764, 0x203C, 0x2049)
    )
...
def ends_properly(answer: str) -> bool:
    ...
    if not stripped:
        return False
    # Эмодзи в конце — тоже законченная мысль: «Всё получится 🌸». Ева пишет
    # так постоянно, а раньше такой ответ считался оборванным — провайдер
    # переспрашивался, потом следующий, потом следующий, и в итоге всё равно
    # отдавался этот же текст. Полностью нормальный разбор стоил нам до шести
    # запросов к ИИ и всего бюджета времени.
    if _is_emoji(stripped[-1]):
        return True
